use crate::board_me::Direction::{DECREASING, INCREASING, STABLE};
...
impl Board {
    ...
    pub fn fmt(&self) {
        //write!(f, "MyStruct {{ value: {} }}", self.tokens)
        for (key, _value) in &self.tokens {
            println!("{:?}: something", key);
        }

        // from these I found out that the hashmap is
        // keeping the pieces that have been play
        // the index is a touple (column, row) for each token
    }
    ...
    pub fn from_number(value: u128) -> Board {
        // converts serialized board into a normal board of numbers
    ...
    pub fn add_token(&mut self, column: u8, color: usize) -> Result<bool, &str> {
    ...
    // pub fn get_color_at_cell(&self, x: u8, y: u8) -> char {
    //     self.tokens.get(&(x, y))
    // }
